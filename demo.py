...
def decode_idx1_ubyte(idx1_ubyte_file):
    with open(idx1_ubyte_file, 'rb') as f:
        print(f'Decoding {idx1_ubyte_file}')
        fb_data = f.read()

    offset = 0
    fmt_header = '>ii'
    magic_number, label_num = struct.unpack_from(fmt_header, fb_data, offset)
    print(f'Magic_num: {magic_number}, label_num: {label_num}')
    offset += struct.calcsize(fmt_header)
    labels = []
    fmt_label = '>B'
...
def check_folder(folder):
    """Check if file and folder is existing, if not then create"""
